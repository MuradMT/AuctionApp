    ...
    :{days: number,hours:number,minutes:number,seconds:number,completed:boolean,}) => {
        return(
            ...
            `}>
                 {
                    completed?(
                    <span className='text-neutral-50 font-semibold'>Auction Finished</span>)
                    :(<span suppressHydrationWarning={true} className='text-neutral-50 font-semibold'>{ zeroPad(days)}:{zeroPad(hours)}:{zeroPad(minutes)}:{zeroPad(seconds)}</span>)
                 }
            </div>
  )};
